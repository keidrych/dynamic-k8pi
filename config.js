 ...
 */
function getInCluster(watchTimeout) {
	const host = process.env.KUBERNETES_SERVICE_HOST
	const port = process.env.KUBERNETES_SERVICE_PORT

	watchTimeout = watchTimeout || 60000
	...
	return {
		...
		namespace,
		timeout: parseInt(watchTimeout, 10)
	}
}
...
function fromKubeconfig(kubeconfig, current, watchTimeout) {
	if (!kubeconfig) {
	...
	return {
		...
		key,
		cert,
		timout: parseInt(watchTimeout, 10)
	}
}
